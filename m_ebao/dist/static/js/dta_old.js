...
(function () {
    ...
    _addEvent(window,'popstate',function(e){
        ...
        params.referrer = getStatisUrl(params.cache, window.eb);
        params.url = getStatisUrl(params.url, window.eb);
        enterTime = new Date().getTime();

        _sendOnLeave('state_leave');
        params.cache = params.url;
    })
    // history api事件监听
    _addEvent(history,'pushstate',function(e){

        params.referrer = getStatisUrl(e.url, window.eb);
        params.url = getStatisUrl(params.url, window.eb);
        enterTime = new Date().getTime();
    ...
    function getStatisUrl(url, eb) {
        if(eb && eb.platform) {
            if(url.match('platform')) {
                url = url.replace(/platform=[A-Za-z]+/g, 'platform=' + eb.platform);
            } else {
                if(url.match('\\?')) {
                    url += '&platform=' + eb.platform;
                } else {
                    url += '?platform=' + eb.platform;
                }
            }
